...
def post_state(
    url: str, token: str, entity_id: str, attributes: dict[str, float]
) -> None:
    payload = json.dumps(
        {
            "state": "not_home",
...
def main() -> int:
    ...
    token = os.environ.get("BIKE_TRACKER_TOKEN", "")
    if not token and not args.dry_run:
        print(
            "BIKE_TRACKER_TOKEN is not set. Create a long-lived access token in "
            "your Home Assistant profile and export it first.",
            file=sys.stderr,
        )
        return 2
    ...
    for index, fix in enumerate(fixes, start=1):
        try:
            post_state(args.url, token, args.entity, fix)
