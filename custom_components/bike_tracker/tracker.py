...
def trip_from_points(
    ...
    for point in points:
        ...
        if (
            point.reported_speed_kmh is not None
            and 0.0 <= point.reported_speed_kmh <= MAX_PLAUSIBLE_SPEED_KMH
        ):
            point.speed_kmh = point.reported_speed_kmh
        else:
            point.speed_kmh = speed_kmh(distance, dt)
...
class TripTracker:
    ...
    def add_point(self, point: GpsPoint) -> None:
        ...
        # Trust the device's own Doppler speed when it looks sane; it is far
        # more accurate than differentiating positions.
        if (
            point.reported_speed_kmh is not None
            and 0.0 <= point.reported_speed_kmh <= MAX_PLAUSIBLE_SPEED_KMH
        ):
